    ...
    for (int i = 0; i < kMaxPlayerNum; i++) {
        if ((a->attributes & kAIsComputer) && (!(a->attributes & kAIsRemote))) {
            if (a->blitzkrieg > 0) {
                ...
                if (a->blitzkrieg <= 0) {
                    // Really 48:
                    a->blitzkrieg = 0 - (XRandomSeeded(1200, &gRandomSeed) + 1200);
                ...
                if (a->blitzkrieg >= 0) {
                    // Really 48:
                    a->blitzkrieg = XRandomSeeded(1200, &gRandomSeed) + 1200;
                    anObject = mGetSpaceObjectPtr(0);
                    for (int j = 0; j < kMaxSpaceObject; j++) {
                        if (anObject->owner == i) {
                            anObject->currentTargetValue = 0x00000000;
                        ...
                        if ((anObject->duty != eEscortDuty)
                            ...
                            if (a->destinationObject >= 0) {
                                ...
                                if (destObject->active == kObjectInUse) {
                                    ...
                                    anObject->currentTargetValue
                                        = anObject->bestConsideredTargetValue;
                                    thisValue = XRandomSeeded(
                                            mFloatToFixed(0.5), &(anObject->randomSeed))
                                        - mFloatToFixed(0.25);
            ...
            if ((anObject->owner == i)
                ...
                if (thisValue > 0) {
                    thisValue += XRandomSeeded(
                            thisValue >> 1, &(anObject->randomSeed)) - (thisValue >> 2);
                }
                    ...
                    if (destBalance->buildTime <= 0) {
                        if (a->hopeToBuild < 0) {
                            ...
                            while ((a->hopeToBuild < 0) && (k < 7)) {
                                ...
                                thisValue = XRandomSeeded(
                                        a->totalBuildChance, &gRandomSeed);
