...
void RealInputSource::key_down(const KeyDownEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new KeyDownEvent(event.at(), event.key())));
}

void RealInputSource::key_up(const KeyUpEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new KeyUpEvent(event.at(), event.key())));
}

void RealInputSource::gamepad_button_down(const GamepadButtonDownEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new GamepadButtonDownEvent(event.at(), event.button)));
}

void RealInputSource::gamepad_button_up(const GamepadButtonUpEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new GamepadButtonUpEvent(event.at(), event.button)));
}

void RealInputSource::gamepad_stick(const GamepadStickEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new GamepadStickEvent(event.at(), event.stick, event.x, event.y)));
}

void RealInputSource::mouse_down(const MouseDownEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new MouseDownEvent(event.at(), event.button(), event.count(), event.where())));
}

void RealInputSource::mouse_up(const MouseUpEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new MouseUpEvent(event.at(), event.button(), event.where())));
}

void RealInputSource::mouse_move(const MouseMoveEvent& event) {
    _events.emplace(make_pair(g.admiral.number(), at()), std::unique_ptr<Event>(new MouseMoveEvent(event.at(), event.where())));
}
...
ReplayInputSource::ReplayInputSource(ReplayData* data):
    ...
    for (auto action: data->actions) {
        ...
        for (auto key: action.keys_down) {
            ...
            _events.emplace(make_pair(0, at), unique_ptr<Event>(new KeyDownEvent(wall_time(), code)));
        }
        for (auto key: action.keys_up) {
            int code = sys.prefs->key(key) - 1;
            _events.emplace(make_pair(0, at), unique_ptr<Event>(new KeyUpEvent(wall_time(), code)));
        }
    }
}
