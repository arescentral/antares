...
static void apply(
    ...
    if (direct->sprite.get()) {
        ...
        int32_t l = (direct->location.h - scaled_screen.bounds.left) * scaled_screen.scale.factor;
        l >>= SHIFT_SCALE;
        ...
        l = (direct->location.v - scaled_screen.bounds.top) * scaled_screen.scale.factor;
